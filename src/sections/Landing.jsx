  ...
  return (
    <>
      ...
      <div style={{
        ...
        {/* Content from top */}
        <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "flex-start", paddingTop: "clamp(24px, 4vh, 48px)" }}>
          <h1 style={{
          ...
          <p style={{
            fontFamily: bd, fontSize: "clamp(15px, 1.3vw, 18px)", fontWeight: 400,
        ...
        <div style={{ flexShrink: 0, paddingBottom: "clamp(16px, 2vh, 32px)" }}>
          ...
          {phase === "sent" ? (
            <div style={{
              padding: "28px 24px", border: `1px solid #2E5A35`,
              textAlign: "center",
            }}>
              <div style={{ fontFamily: bd, fontSize: 18, fontWeight: 600, color: t.cream, marginBottom: 8 }}>
                Check your email
              </div>
              <p style={{ fontFamily: bd, fontSize: 14, color: t.faint, lineHeight: 1.6, margin: 0 }}>
                We sent a magic link to <strong style={{ color: t.cream }}>{email}</strong>. Click it to access the dashboard.
              </p>
              <button
                ...
                style={{
                  ...
                  fontSize: 12, fontFamily: bd, marginTop: 12,
                }}
              >
                Try again
              </button>
            ...
            <>
              ...
              <div style={{ display: "flex", border: `1px solid ${t.border}` }}>
                ...
                <div
                  ...
                  style={{
                    padding: "12px 20px", borderLeft: `1px solid ${t.border}`,
              ...
              {phase === "error" && (
                <div style={{ fontFamily: bd, fontSize: 12, color: "#EB5757", marginTop: 8 }}>{errorMsg}</div>
              )}

              <div style={{ fontFamily: bd, fontSize: 11, color: t.borderFaint || "#3A3B36", marginTop: 10 }}>
      ...
      <div ref={whatRef} style={{
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 28 }}>
          {[
            ...
            <div key={item.title}>
              <div style={{ fontFamily: bd, fontSize: "clamp(16px, 1.5vw, 19px)", fontWeight: 500, color: t.cream, marginBottom: 6 }}>
                {item.title}
              </div>
              <div style={{ fontFamily: bd, fontSize: "clamp(14px, 1.3vw, 16px)", color: t.faint, lineHeight: 1.65 }}>
                {item.desc}
              </div>
            </div>
          ))}
      ...
      <div ref={hoodRef} style={{
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          {[
            {
              title: "Power Law — WLS + RANSAC",
              desc: "Weighted Least Squares on 16 years of daily data with 4-year recency decay. RANSAC for a robust support floor that excludes bubble peaks. EVT/Generalized Pareto Distribution for the empirical upside cap.",
            },
            {
              title: "Fractal volatility — MMAR",
              desc: "Mandelbrot's Multifractal Model of Asset Returns with Hurst exponent via DFA and multifractal partition function. Captures fat tails, volatility clustering, and long memory that Gaussian models miss entirely.",
            },
            {
              title: "Regime detection — OU diagnostic",
              desc: "Ornstein-Uhlenbeck process with two regimes (calm and volatile). Used as a regime diagnostic — not as the MC engine. The simulation runs on pure MMAR/Hurst dynamics to avoid artificially dampening tail risk.",
            },
            {
              title: "Monte Carlo — 2,000 paths, 3-year horizon",
              desc: "Fractal cascades, empirical shock resampling, Hurst-correlated noise. Single unified run — 1Y and 3Y percentiles extracted from the same paths. RANSAC reflective floor with empirically calibrated break probability.",
            },
            {
              title: "Signal calibration — walk-forward backtest",
              desc: "Buy score weights and sell thresholds calibrated by grid search against historical returns. Two independent sell paths. Probabilistic calibration table shows how well MC loss estimates matched reality across every market cycle.",
            },
          ].map((item) => (
            <div key={item.title} style={{ display: "flex", gap: 14 }}>
              <div style={{ width: 2, background: t.border, flexShrink: 0, alignSelf: "stretch" }} />
              <div>
                <div style={{ fontFamily: bd, fontSize: "clamp(14px, 1.3vw, 16px)", fontWeight: 500, color: t.cream, marginBottom: 4 }}>
                  {item.title}
                </div>
                <div style={{ fontFamily: bd, fontSize: "clamp(13px, 1.1vw, 15px)", color: t.faint, lineHeight: 1.6 }}>
                  {item.desc}
                </div>
              </div>
            </div>
        ...
        <div style={{
          ...
          textAlign: "center", marginTop: 36, paddingTop: 24,
          borderTop: `1px solid ${t.border}`,
        }}>
          {[
            ...
            <div key={label}>
              <div style={{ fontFamily: mn, fontSize: 22, fontWeight: 700, color: t.cream }}>{value}</div>
              <div style={{ fontFamily: bd, fontSize: 11, color: t.faint, marginTop: 3 }}>{label}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{
        padding: "16px 0 12px",
        fontFamily: bd, fontSize: 12, color: t.borderFaint || "#3A3B36", lineHeight: 1.6,
      }}>
        <span style={{ color: t.faint }}>Not financial advice.</span> Past signal accuracy doesn't guarantee future results. Bitcoin is volatile and the model can be wrong. Use this as a structured framework for thinking about risk — not as a trading system. The 98% accuracy figure is historical and based on data the model was partially trained on.
      </div>
      ...
      <div style={{
        ...
        fontFamily: bd, fontSize: 11, color: t.borderFaint || "#3A3B36",
      }}>
