  ...
  if (Math.abs(sig) < 0.15) {
    return (
      <div style={{ padding: "16px 0", fontFamily: bd, fontSize: 15, fontWeight: 500, color: "#27AE60" }}>
        At fair value — no active episode.
      </div>
    );
  }
    ...
    <>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 16 }}>
        ...
        <div style={{ textAlign: "right" }}>
          <span style={{ fontFamily: mn, fontSize: 10, color: t.faint, padding: "2px 8px", border: `1px solid ${t.borderFaint}`, borderRadius: 10 }}>
            {sigImproving ? "σ ↑ improving" : sigWorsening ? "σ ↓ worsening" : "σ → flat"}
          </span>
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", marginTop: 4 }}>
          ...
          <span style={{ fontFamily: mn, fontSize: 9, color: t.cream, fontWeight: 600 }}>median: {episodeHistory.median}d</span>
      ...
      <div className="data-grid-4" style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 0, borderBottom: `1px solid ${t.borderFaint}`, marginBottom: 16 }}>
        {[
          { label: "Optimistic", value: conditionalFast > 0 ? `${Math.round(conditionalFast / 30)}m` : "now", sub: conditionalFast > 0 ? `${conditionalFast}d left` : "overdue" },
          { label: "Expected", value: conditionalRemaining > 0 ? `${Math.round(conditionalRemaining / 30)}m` : "now", sub: conditionalRemaining > 0 ? `${conditionalRemaining}d left` : "overdue" },
          { label: "Conservative", value: conditionalSlow > 0 ? `${Math.round(conditionalSlow / 30)}m` : "now", sub: conditionalSlow > 0 ? `${conditionalSlow}d left` : "overdue" },
        ].map((c, i) => (
          <div key={c.label} style={{ padding: "12px 0", textAlign: "center", borderRight: i < 2 ? `1px solid ${t.borderFaint}` : "none" }}>
            <div style={{ fontFamily: bd, fontSize: 9, color: t.faint, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 4 }}>{c.label}</div>
            <div style={{ fontFamily: mn, fontSize: 22, fontWeight: 700, color: t.cream }}>{c.value}</div>
            <div style={{ fontFamily: bd, fontSize: 9, color: t.faint, marginTop: 2 }}>{c.sub}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ fontFamily: bd, fontSize: 9, color: t.faint, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 8 }}>
        Previous {episodeHistory.label} episodes
      </div>
      {episodeHistory.durations.map((d, i) => (
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
        ...
        <span style={{ width: 30, fontFamily: mn, fontSize: 10, color: sig < 0 ? "#2F80ED" : "#F2994A", fontWeight: 700 }}>now</span>
      </div>
    </>
  );
}
